...
<template>
  <div class="chat-edit">
    ...
    <div class="msg-input">
      ...
      >
        <template #reference>
          <div class="action" @mouseover="isHovered = true" @mouseleave="isHovered = false">
            <Icon v-if="isHovered" icon="face" :size="19" colorful />
            <Icon v-else icon="face" :size="19" colorful />
          </div>
        </template>
      ...
      />
      <Icon class="action" icon="file" :size="22" colorful @click="openFileSelect('file')" />
      <div class="divider" />
      <div
        :class="['action', { 'is-edit': inputMsg.length, 'disabled': !inputMsg.length }]"
        @click="sendMsgHandler"
      >
        <Icon class="send" icon="send" :size="20" colorful />
      </div>
    </div>
